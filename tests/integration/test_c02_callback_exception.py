...
def test_c02_callback_exception_clears_the_outbound_sale(c02_run):
    result, ledger, _ = c02_run
    assessment = result.assessment
    events = ledger.events(result.run_id)
    types = [event.type.value for event in events]
    assert result.status == "complete"
    assert result.termination.value == "assessment_complete"
    assert assessment.route.route_id == "callback_sale_exception"
    ...
    assert len([event for event in events if event.type.value == "tool_call"]) <= 8
    assert events[-1].type.value == "termination"
    assert ledger.verify_chain(result.run_id)


def test_c02_tool_node_and_assessment_transparency_reconcile(c02_run):
    result, ledger, path = c02_run
    events = ledger.events(result.run_id)
