...
def test_c02b_callback_exception_correctly_fails(c02b_run):
    result, ledger, _ = c02b_run
    ...
    assert finding.category == "MC-07"
    assert finding.status == "substantiated"
    assert finding.attributable_to == "colleague"
    assert finding.severity == "medium"
    assert assessment.customer_outcome["remediation"] == [{"action": "notify_and_offer_reversal"}]
...
def test_c02b_tool_node_and_assessment_transparency_reconcile(c02b_run):
    result, ledger, path = c02b_run
    events = ledger.events(result.run_id)
