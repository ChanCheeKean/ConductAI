...
Current phase: **Stage 4C — temporal evidence, reconciliation, sandbox, verifier/re-plan: COMPLETE; awaiting user checkpoint**
Next stage: **Stage 4D — graph memory, cross-channel and specialists (C02/C02b, C08, C15, C16)**

## Current objective

Build **ConductAI**, a proof-of-concept, fully automated agent system that reviews credit-card customer-service interactions (phone, chat, secure message) at the fictional **Copperlake Bank, N.A.** for sales and servicing misconduct. Stage 4 was split into smaller checkpointed substages because the implementation phase is large. Stages 4A–4C now prove the first vertical slice plus generalization: C03's deterministic L1 early path, C01's bounded Deep Agents L2 investigation with private artifact scheduling and durable wait/resume, C05's said/offered/billed reconciliation with sandbox fee arithmetic and precedent distinguishing, C07b's clock-skew event-time reconciliation, C19's memory-vs-glossary supersession, and C04's mid-investigation root-cause re-plan (a real LangGraph back-edge) from individual misconduct to a systemic script-attributable control gap resolved by a real panel (advocates, adjudicator, computed confidence). The router, tool catalog and data repository are now generalized across six routes instead of hardcoded per-case. The next action is the user checkpoint before Stage 4D adds graph memory and cross-channel specialists.
...
8. `config/models.yaml`, `config/routes.yaml`, `config/scenarios/{c01,c03,c04,c05,c07b,c19}.yaml` — implemented registries.
9. `src/conductai/adapters/runtime/langgraph_runtime.py`, `src/conductai/runtime/{c01,c03,c04,c05,c07b,c19}_workflow.py`, `src/conductai/runtime/{sandbox,support}.py`, and `src/conductai/harness/artifacts.py` — current vertical slices.
...
| `config/`, `skills/{credit-line-increase,add-on-consent,balance-transfer-disclosure,memory-hygiene}/` | Validated model/route/scenario registries (6 routes) and four Deep Agents-format skills | Stage 4C complete |
| `src/conductai/` | Domain models, config, guarded data, typed tools (18 registered), generalized router, runtime protocols, conditional LangGraph adapter with a real re-plan back-edge, bounded Deep Agents lead, private artifact harness, C01/C03/C05/C07b/C19/C04 workflows, shared `sandbox`/`support` helpers, event ledger/blobs/schema/replay and CLI | Stage 4C complete |
| `docs/schemas/` | JSON Schema exports for the 79-type v1 event envelope and assessment record | Stage 4A complete; unchanged in 4C (no new Pydantic fields) |
| `tests/{architecture,contract,unit,integration}/` | Import boundaries, gateway/config/access/ledger contracts plus C03, C01, C05, C07b, C19 and C04 trajectory/replay/checkpoint acceptance (34 tests total) | Stage 4C complete |
...
4. **Stage 4D NEXT:** graph memory, cross-channel and specialists: C02/C02b, C08, C15, C16.
...
- Stages 4A–4C deliberately implement only C03, C01, C05, C07b, C19 and C04's graph paths. The fallback route fails closed; it must not be treated as a general reviewer until later substages land.
...
- C05, C07b, C19 and C04 make no model call (same deterministic-rule-match reasoning as C03). The lead Deep Agent has now been exercised only on C01; whether every future L3/L4 case needs a real model call, or can stay code-owned like C04, is a per-case design decision for later stages, not a general rule.
- `run_registered_query`'s two C04 templates and `search_precedents`/`search_transcripts`'s FTS5 queries are the first registered-SQL and hybrid-retrieval tools actually exercised end-to-end; they cover exactly the phrasing present in the generated data (verified against `conduct.sqlite` directly) and are not yet stress-tested against a larger or more varied population.
