"""C02b wrong callback: the same callback-exception check correctly fails because the fulfilled sale's product
does not match the purpose the customer actually requested; contrast pair with C02."""
...
class C02bWorkflow:
    """Deterministic outbound-sale callback-exception-failure path; C02b only."""
    ...
    def route(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        if route.route_id != "callback_sale_exception":
            raise RuntimeError("C02b requires the callback_sale_exception route")
    ...
    def integrity(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        transcript, used = self.tools.execute(
            ...
            rationale="Read the exact unrelated-product pitch and acceptance",
    ...
    def gather(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        request, used = self.tools.execute(
            ...
            rationale="Confirm what was actually sold on this outbound call",
            used=used, limit=state["route"]["budget"]["tool_calls"], **node_context(state),
        )
        inquiry, used = self.tools.execute(
            "get_bureau_inquiry", {"credit_request_id": request["credit_request_id"]},
            rationale="Confirm the resulting inquiry to establish customer harm, if any",
            used=used, limit=state["route"]["budget"]["tool_calls"], **node_context(state),
        )
        evidence = {**state["evidence"], "chain": link, "callback": callback,
                    "credit_request": request, "inquiry": inquiry}
        blob = self.ledger.put_blob(evidence)
        self.ledger.emit(
            **node_context(state), actor=Actor(kind="graph_node", name="review_file"),
            type=EventType.REVIEW_FILE_UPDATED, summary="Added the callback chain, callback record, and CLI request",
            payload={"path": "evidence_matrix.json", "patch_blob": blob, "columns": ["said", "callback", "sold"]},
            refs=[link["created_in_interaction_id"], callback["callback_request_id"], request["credit_request_id"]],
        )
    ...
    def reconcile(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        purpose_matches = callback["purpose"] == "credit_line_increase"
        ...
        finding = self.ledger.emit(
            **node_context(state), actor=Actor(kind="graph_node", name="records_reconciler"),
            ...
            summary=f"The callback purpose was {callback['purpose']!r}, not credit_line_increase — the outbound "
                    "sale falls outside the callback exception",
            payload={"finding_id": "F1", "status": "substantiated_candidate" if not exception_holds else "no_error_candidate",
    ...
    def preverify(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        checks = {
            "purpose_mismatch_confirmed": not state["purpose_matches"],
            "consent_recorded": state["consent_recorded"],
            "exception_fails": not state["exception_holds"],
            "inquiry_soft": evidence["inquiry"]["inquiry_type"] == "SOFT",
        }
        span = self.ledger.emit(
            **node_context(state), actor=Actor(kind="governance", name="deterministic_verifier"),
            type=EventType.EVIDENCE_SPAN_VERIFIED, summary="Verified the exact unrelated-product pitch turn",
        ...
        for check_id, passed in checks.items():
            event = self.ledger.emit(
                **node_context(state), actor=Actor(kind="governance", name="deterministic_verifier"),
                ...
                payload={"check_id": check_id, "kind": "C02b_L2", "result": "pass" if passed else "fail"},
                refs=[evidence["callback"]["callback_request_id"]],
            )
            check_seqs.append(event.seq)
        if not all(checks.values()):
            raise RuntimeError("C02b deterministic verifier failed")
    ...
    def panel_gate(self, state: dict[str, Any]) -> dict[str, Any]:
        return {"panel_used": False,
                "panel_reason": "medium severity, no customer harm (soft inquiry, customer accepted), "
                                "remediation below $250 with high confidence",
                "panel_event_seqs": []}
    ...
    def decide(self, state: dict[str, Any]) -> dict[str, Any]:
        event = self.ledger.emit(
            **node_context(state), actor=Actor(kind="governance", name="outcome_gate"),
            ...
            summary="Proposed MC-07 substantiated: the outbound sale did not match the callback's purpose",
            payload={"finding_id": "F1", "category": "MC-07", "status": "substantiated", "attributable_to": "colleague"},
    ...
    def record(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        source_refs = [f"{interaction_id}:t01", callback["callback_request_id"], request["credit_request_id"]]
        seqs = sorted(set([
        ...
        finding = Finding(
            finding_id="F1", category="MC-07", status="substantiated", attributable_to="colleague", severity="medium",
            ...
            structured_evidence=[
                {"source": "callback_requests", "id": callback["callback_request_id"],
                 "fact": f"purpose={callback['purpose']}, not credit_line_increase"},
                {"source": "credit_line_requests", "id": request["credit_request_id"],
                 "fact": f"soft inquiry, decision={request['decision']}"},
            ],
        ...
        base: dict[str, Any] = {
            ...
            "customer_outcome": {"harm_likely": False, "remediation": [{"action": "notify_and_offer_reversal"}]},
            ...
            "adjudication": {"panel_used": False, "panel_reason": state["panel_reason"],
                             ...
                             "flip_fact": "the callback purpose had actually been credit_line_increase"},
            ...
            "graph_writes": [], "hypotheses": [
                {"id": "H1", "label": "permitted under the callback exception", "status": "rejected",
                 "evidence_against": source_refs},
                {"id": "H2", "label": "prohibited outbound sale unrelated to the customer's requested callback purpose",
                 "status": "supported", "evidence_for": source_refs},
            ], "citations": [{"doc_id": "CLB-SOP-SAL-002@v3", "why": "outbound-sale callback exception"}],
            "summary_for_record": "MC-07 substantiated. The outbound call sold a credit line increase, but the "
                                   f"customer's callback request was for {callback['purpose']}, so the sale falls "
                                   "outside the callback exception.",
        ...
        event = self.ledger.emit(
            **node_context(state), actor=Actor(kind="graph_node", name="assessment_repository"),
            type=EventType.ASSESSMENT_RECORDED, summary="Recorded provenance-complete C02b assessment",
