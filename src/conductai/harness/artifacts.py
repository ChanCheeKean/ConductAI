...
class ArtifactHarness:
    ...
    def __init__(self, dataset_root: Path, run_database: Path, ledger: EventLedger) -> None:
        ...
        personas_path = self._dataset_root / "simulation" / "customer_personas.json"
        self._personas = {row["review_id"]: row for row in json.loads(personas_path.read_text(encoding="utf-8"))}
    ...
    def request_customer_message(
        self, *, review_id: str, customer_id: str, question: str, respond_by: str,
        idempotency_key: str, run_id: str, virtual_now: datetime,
    ) -> dict[str, Any]:
        persona = self._personas.get(review_id)
        if persona is None or persona["customer_id"] != customer_id:
            raise LookupError("no outreach persona registered for this review and customer")
        artifact_id = f"OUTREACH-{review_id}"
        expected = _parse(persona["reply_to_outreach"]["available_at"])
        latest = _parse(respond_by)
        if latest < virtual_now:
            raise ValueError("outreach deadline is before the current virtual time")
        request = {
            "artifact_id": artifact_id, "interaction_id": review_id, "kind": "customer_reply",
            "respond_by": _iso(latest), "idempotency_key": idempotency_key,
        }
        with sqlite3.connect(self._database) as conn:
            prior = conn.execute(
                "SELECT request_json,expected_at FROM artifact_requests WHERE run_id=? AND idempotency_key=?",
            ...
            if prior is not None:
                if json.loads(prior[0]) != request:
                    raise ValueError("idempotency key reused with different outreach request")
                return {**request, "requested_at": _iso(virtual_now),
                        "expected_at": prior[1], "status": "already_requested"}
            conn.execute(
                "INSERT INTO artifact_requests VALUES(?,?,?,?,?,?,?,?,?,?,NULL)",
                (run_id, artifact_id, review_id, review_id, "customer_reply", idempotency_key,
                 _iso(virtual_now), _iso(expected), _iso(latest), canonical_json(request)),
            )
        self._ledger.emit(
            run_id=run_id, review_id=review_id, virtual_now=virtual_now,
            actor=Actor(kind="harness", name="artifact_scheduler"), type=EventType.CUSTOMER_OUTREACH_SENT,
            summary=f"Sent outreach question to {customer_id}",
            payload={**request, "question": question, "expected_at": _iso(expected), "release_rule": "fixed_available_at"},
            refs=[artifact_id, customer_id],
        )
        return {**request, "requested_at": _iso(virtual_now),
                "expected_at": _iso(expected), "status": "requested"}
        ...
        with sqlite3.connect(self._database) as conn:
            ...
            if arrived:
                if row["kind"] == "customer_reply":
                    persona = self._personas[row["review_id"]]
                    artifact = {"text": persona["reply_to_outreach"]["template"]}
                else:
    ...
    def schedule_follow_up(
        self, *, review_id: str, at: str, action_type: str, idempotency_key: str,
        run_id: str, virtual_now: datetime,
    ) -> dict[str, Any]:
        with sqlite3.connect(self._database) as conn:
            prior = conn.execute(
                "SELECT at,action_type FROM follow_up_requests WHERE run_id=? AND idempotency_key=?",
                (run_id, idempotency_key),
            ).fetchone()
            if prior is not None:
                if prior != (at, action_type):
                    raise ValueError("idempotency key reused with a different follow-up")
                return {"review_id": review_id, "at": at, "action_type": action_type, "status": "already_scheduled"}
            conn.execute(
                "INSERT INTO follow_up_requests VALUES(?,?,?,?,?,?)",
                (run_id, review_id, idempotency_key, at, action_type, _iso(virtual_now)),
            )
        return {"review_id": review_id, "at": at, "action_type": action_type, "status": "scheduled"}
